...
export interface DCMotor extends ClassHandle {
  ...
  currentFromSpeedAndVoltage(speedRadPerSec: number, inputVoltageVolts: number): number;
  currentFromTorque(torqueNewtonMeters: number): number;
  torqueFromCurrent(currentAmps: number): number;
  voltageFromTorqueAndSpeed(torqueNewtonMeters: number, speedRadPerSec: number): number;
  speedFromTorqueAndVoltage(torqueNewtonMeters: number, inputVoltageVolts: number): number;
...
interface EmbindModule {
  ...
  DCMotor: {
    new(_0: number, _1: number, _2: number, _3: number, _4: number, _5: number): DCMotor;
  };
  ElevatorSim: {
  ...
  BatterySim_calculateDefaultBatteryLoadedVoltage(currentDrawsAmps: VectorDouble): number;
  BatterySim_calculateLoadedBatteryVoltage(nominalVoltageVolts: number, resistanceOhms: number, currentDrawsAmps: VectorDouble): number;
}
