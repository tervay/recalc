...
// Hard cap so a pathological current-limit combination (e.g. iMax = 0, which
// stalls the sim's velocity forever) can never hang the worker. A normal
// motor curve completes in well under 10,000 iterations, even at unusually
// low but nonzero current limits.
...
export async function generateMotorCurve(
  ...
  const resistance = new Measurement(wpilibMotor.getROhms(), 'Ohm');
  ...
  try {
    ...
    let iterations = 0;
    while (
      new Measurement(motorSim.getAngularVelocity(), 'rad/s').lte(
        motor.freeSpeed,
      ) &&
