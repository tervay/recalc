...
test.describe('Copy Link', () => {
  ...
  test('tracking: includes motor name when copying link on a motor calculator', async ({
    page,
  }) => {
  ...
  test('tracking: does not include motor when copying link on a non-motor calculator', async ({
    page,
  }) => {
    const calls = await setupTrackCapture(page);
